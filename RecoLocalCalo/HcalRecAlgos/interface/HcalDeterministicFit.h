...
class HcalDeterministicFit {
 public:
  ...
  template<class Digi> void apply(const CaloSamples & cs, const std::vector<int> & capidvec, const HcalCalibrations & calibs, const Digi & digi, std::vector<double> & HLTOutput) const {
    ...
    float n3=0;
    getLandauFrac(-tsShift3+25,-tsShift3+tsWidth*2,n3);
    float nn3=0;
    getLandauFrac(-tsShift3+50,-tsShift3+tsWidth*3,nn3);
    
    float i4=0;
    getLandauFrac(-tsShift4,-tsShift4+tsWidth,i4);
    float n4=0;
    getLandauFrac(-tsShift4+25,-tsShift4+tsWidth*2,n4);

    float i5=0;
    getLandauFrac(-tsShift5,-tsShift5+tsWidth,i5);
    float n5=0;
    getLandauFrac(-tsShift5+25,-tsShift5+tsWidth*2,n5);
